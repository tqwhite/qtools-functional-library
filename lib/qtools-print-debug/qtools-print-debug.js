...
var moduleFunction = function(args = {}) {
	...
	function qtDump(args={}) {
		...
		let returnn = '';
		let showLabel = '';
		const recursionDepth=typeof(args.recursionDepth)!='undefined'?args.recursionDepth:null;
		const addCliColors=typeof(args.addCliColors)!='undefined'?args.recursionDepth:true;
		...
		if (['string', 'number', 'boolean'].includes(typeof this)) {
			if (returnString) {
				...
				console.log(
					`${separator}${
						label ? showLabel + '= ' : ''
					}${outString} [${callerName}]${separator}${suffix}`
				);
			}
			...
			if (returnString) {
				return (
					util.inspect(tmpObject, { depth: recursionDepth, maxArrayLength: null,
							colors: addCliColors }) +
					` [${callerName}]`
				);
			} else {
